...
public class Controller {
    ...
    private Land<ArrayList<Person>> setupPeople() {
        ...
        // creating random person land until the distribution of class fulfills
        // the objective (every class has at least CLASS_THRESHOLD percent of
            ...
            for (int j = 0; j < Params.LAND_SIZE; j++) {
                ...
                for (int k = 0; k < people.size(); k++) {
                    ...
                    if (people.get(k).isDead()) {
                        // Extension: when a person die, his offspring will
                        // inherit a proportion of his wealth
                        int legacy = people.get(k).getWealth();
                        nextPersonLand.getValue(i, j)
                        	.add(new Offspring(legacy));
    ...
    /*
     * Round the grain up after diffusion
     */
